...
import { FaCheckCircle, FaCheckSquare, FaClock, FaExclamationTriangle } from "react-icons/fa";
...
const AnswerChoice = ({ color, text }) => (
  <div className="flex items-start mb-2 w-1/2">
    <span className={`w-4 h-4 rounded-full my-1 mr-2 shrink-0 relative bg-${color}-600`} style={{backgroundColor:color}} />
...
const Slider = () => {
  const choices = [
    { color: "red", text: "tùy" },
    { color: "red", text: "hồ" },
    { color: "green", text: "khác" },
    { color: "red", text: "khổ" },
  ];

  return (
    
    <div className="question-details-card flex flex-col border border-solid border-dark-6 rounded-lg mb-4 rounded cursor-pointer bg-light-3 hover:bg-light-2">
        ...
        <div className="flex justify-between h-6 mb-4">
          <div className="flex flex-col xs:flex-row gap-2 xs:gap-0 justify-between grow">
            <span className="inline-flex items-center min-w-max">
              <div className="lozenge items-center inline-flex text-xs font-semibold py-0.5 rounded px-1.5">
                <span className="mr-2">
                  <FaCheckSquare className="flex items-center far fa-check-square" style={{ fontSize: 11 }} />
                </span>
                <span>2. Multiple Choice</span>
              </div>
            </span>
          </div>
          <div className="ml-auto h-full self-start flex">
            <button
              ...
              type="button"
            >
              <FaExclamationTriangle className="far fa-exclamation-triangle" style={{ fontSize: 11 }} />
            </button>
            <Lozenge iconClass={FaClock} text="30 seconds" />
            <Lozenge iconClass={FaCheckCircle} text="1 point" />
          </div>
        </div>
        <div className="flex gap-2 items-center mb-4">
          <div className="question-wrapper text-sm flex overflow-hidden w-full text-dark-2 items-center">
            <p>Thua keo này, bày keo ...</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap">
        {choices.map((choice, index) => (
          <AnswerChoice key={index} color={choice.color} text={choice.text} />
        ))}
      </div>
    </div>
    </div>
