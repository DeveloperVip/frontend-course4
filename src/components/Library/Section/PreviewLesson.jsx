...
const PreviewLesson = () => {
  ...
  // console.log("🚀 ~ PreviewLesson ~ param :", param);
  const navigate = useNavigate();
  const [indexSlide, setIndexSlide] = useState(0);
  const [isOpen, setIsOpen] = useState(true);
  const [showCorrectAnswer, setShowCorrectAnswer] = useState(false);
  const { slides } = useContext(LessonContext);
  const { lesson} = useFetchAPILessonById(param?.id);
  const [slideShow, setSlideShow] = useState({});
  // console.log("🚀 ~ PreviewLesson ~ lesson:", lesson);
  useEffect(() => {
    if (lesson?.question) {
      setSlideShow(lesson.question[indexSlide]);
    }
  }, [indexSlide, lesson]);
        ...
        <div className="relative flex items-center justify-between bg-black text-white p-2 font-bold rounded-lg">
          <div className="flex">
            <div className="fraction-text min-w-16 px-3 py-2 bg-light-10% rounded-lg text-xs flex justify-center items-end shrink">
              <span className="text-lg">{indexSlide + 1}</span> /
          ...
          <div className="flex items-center h-full">
            <div className="flex flex-col items-center ml-3">
              ...
              <button
                ...
                type="button"
                onClick={() => {
                  if (indexSlide < lesson?.question?.length - 1)
